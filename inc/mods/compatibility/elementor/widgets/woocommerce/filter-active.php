...
class Filter_Active extends The7_Elementor_Widget_Base {
	...
	protected function display_items( $settings ) {
		...
		$branding_filter  = isset( $_GET['filter_product_brand'] ) ? array_filter( array_map( 'absint', explode( ',', wp_unslash( $_GET['filter_product_brand'] ) ) ) ) : array(); // WPCS: sanitization ok, input var ok, CSRF ok.
		...
		if ( ! $this->isPreview() ) {
			if ( ! ( 0 < count( $_chosen_attributes ) || 0 < $min_price || 0 < $max_price || ! empty( $rating_filter ) || ! empty( $branding_filter ) ) ) {
				return false;
			}
		}
		...
		<ul class="filter-nav">
			...
			if ( ! empty( $branding_filter ) ) {
				foreach ( $branding_filter as $brand_id ) {
					$brand = get_term_by( 'id', $brand_id, 'product_brand' );
					if ( ! $brand || is_wp_error( $brand ) ) {
						continue;
					}

					$link_branding = implode( ',', array_diff( $branding_filter, array( $brand_id ) ) );
					$link         = $link_branding ? add_query_arg( 'filter_product_brand', $link_branding ) : remove_query_arg( 'filter_product_brand', $base_link );

					if ( ! $link_branding ) {
						$link = remove_query_arg( 'filtering', $link );
					}

					$brand = get_term_by( 'id', $brand_id, 'product_brand' );

					$this->display_term_html( $link, [ 'active' ], esc_html( $brand->name ), $settings, 'active' );
					$items++;
				}
				$remove_all_filters_link = remove_query_arg( 'filter_product_brand', $remove_all_filters_link );
				$remove_all_filters_link = remove_query_arg( 'filtering', $remove_all_filters_link );
			}
