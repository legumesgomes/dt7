...
class The7_Remote_API {
	...
	const THEME_PURCHASE_URL = 'https://themeforest.net/checkout/from_item/5556590?license=regular&support=bundle_6month&ref=Dream-Theme';
	...
	public function register_purchase_code() {
		...
		if ( '200' != $response_code ) {
			return new WP_Error( 'bad_request', $response_code . ': ' . $this->get_string( 'bad_request' ) );
		}
		...
		if ( empty( $code_check['success'] ) ) {
			return new WP_Error( 'invalid_response', $this->get_string( 'invalid_response' ) );
		}
	...
	public function de_register_purchase_code() {
		...
		if ( '200' != wp_remote_retrieve_response_code( $response ) ) {
			return new WP_Error( 'bad_request', $this->get_string( 'bad_request' ) );
		}
	...
	public function download_demo( $id, $target_dir, $req_url = '' ) {
		...
		if ( ! $wp_filesystem && ! WP_Filesystem() ) {
			return new WP_Error( 'fs_unavailable', $this->get_string( 'fs_unavailable' ) );
		}
		...
		if ( ! is_array( $remote_response ) || 200 !== $response_code ) {
			return new WP_Error(
				'download_failed',
				sprintf( $this->get_string( 'download_failed' ), $response_code )
			);
		}
		...
		if ( ! is_dir( $dummy_dir ) ) {
			return new WP_Error( 'fs_no_folder', sprintf( $this->get_string( 'fs_no_folder' ), $dummy_dir ) );
		}
